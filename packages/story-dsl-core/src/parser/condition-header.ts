...
export function parseConditionHeader(
  line: ParsedLine,
  keyword: string,
  zeroArgumentPredicates: ReadonlySet<string> = new Set(),
): ParsedConditionHeader {
  ...
  const result = parseExpression(rawCondition, position(line, startColumn + 1), { zeroArgumentPredicates });
  return {
