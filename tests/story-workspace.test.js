    ...
    segments: [
      {
        name: "start",
        steps: [
          { kind: "choice", prompt: { speaker: "主角", text: "去哪" }, options: [
...
test("JSON 无损比较忽略对象字段顺序", () => {
  const result = findJsonDifferences(
    { version: 1, segments: [{ name: "start", steps: [] }] },
    { segments: [{ steps: [], name: "start" }], version: 1 },
  );
...
test("JSON 无损比较报告未知字段、数组顺序和值类型", () => {
  const result = findJsonDifferences(
    { version: 1, extra: true, values: ["a", 2] },
    { version: "1", values: [2, "a"] },
  );
